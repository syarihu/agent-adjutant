...
開いただけで指示までは運んでいない（「親タスクの hub を提案する」）。やるのは**起票まで**で、
**着手させない。**

**足す形になる。** 親は既にサブタスクを持っていることがある。既存分は「起動時に読む」で出した
一覧そのものなので、**同じものをもう一度作らない**。まだ収集が返っていなければ先に待つ
（「集計中なのだ」でターンを終える。「待機の作法」）。
...
**承認は1回にまとめる。** 案を全部見せて `AskUserQuestion` を1度だけ開く。選択肢は「このまま起票 /
直す / やめる」で、直すと言われたら直した案をもう一度見せる。**N件ぶん聞かない** — 1件ずつ承認を
取る hub は使い物にならない。**開かないのは起動時の話**（「起動時に読む」）で、ここは人が目の前にいる。

**起票は「依頼が届いたら」の Step 3 をそのまま使う。ここに写さない。** `issueCreate.command` を
`Skill` で呼ぶのも、`notes` を読んでから呼ぶのも、起票コマンドが無い `type` の分岐も同じ。
**この経路で違うのはこれだけ**:

- **起票先は親と同じソース**（冒頭の逆引きで決まったエントリ）。あちらの既定は報告の発見元だが、
  ここに報告は無い。
- **親はこの hub の識別子。** 起票コマンドに親を聞かれたらそれを答える。あちらの「独立した issue に
  するか sub-issue にするか」は判断しない — 割ると言われた時点で、全部この親の下。`jira` を自分で
  起票するときに `parent` へ渡せるのがサブタスク相当のタイプだけなのも、同じく Step 3。
- **Step 2（重複を探す）の相手は、上で突き合わせた親の下の一覧。** repo 全体の検索は掛けない。
  親の外に似たものがあるかは見ていないので、心当たりは案を見せるときに1行添える。
- **Step 5（返信）は要らない。** 依頼元はこのタブの人で、結果はその場で出す。
- **ヒアリングを埋めるのは承認済みの案**（あちらは報告）。承認は上で取ってあるので、案に書いて
  あることを聞き直さない。

**起票したら止まる。** 1件ずつ起票して、そのたびに**キーと URL を1行**出す。全部終わったら
「{N}件起票したのだ。着手はまだなのだ」と書いて**待機に戻る**。

- **「依頼が届いたら」の Step 4 へ進まない。** 着手の引き金は人のまま。
- **一覧に手で継ぎ足さない。** 起票したぶんに番号を振らない。「着手へ渡すとき」が読むのは収集の
  機械行で、**起票したばかりのサブタスクはそこに無い**（「起動時に読む」）。
- **その場でキーを言われたら、先に「1. 一覧」を出し直す。** 収集が拾えば機械行が付き、そこから
  いつもどおり着手へ渡せる。hub は状態を持たないので、待つのは次の収集1回だけ。
...
「割ってくれと言われたら」へ。** あちらは起票して止まる経路で、着手まで走る 3 とは別
（「これ起票して着手して」と言われたときは 3 のまま）。
