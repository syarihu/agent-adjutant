...
要るのは 症状 / 該当箇所 (file:line) / 発見元（依頼元がいま持っているタスク）。
...
**発見元と親タスクの番号は突き合わせる**（親タスクが `-` なら発見元だけ）。タイトルを1つずつ
引いて（`github` 系なら `gh issue view <n> -R <tracker repo> --json title`、`jira` なら
`getJiraIssue` の `summary`）、報告が書いている説明と噛み合うか見る。噛み合わなければ番号の
