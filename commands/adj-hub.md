...
それぞれの URL から割り出す** — 報告はどちらも URL で持っている（`## 発見元` も `## 親タスク` も）ので、
発見元の道具を親タスクに使い回さない。ボードは複数の repo の issue を載せるので、親が別 repo
（`jira` なら別ホスト）にいることがあり、発見元の repo に同じ番号の別タスクがあれば**エラーも
出さずに**そちらが返る。そのうえでタイトルを1つずつ引いて（`github` 系なら
