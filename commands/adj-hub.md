...
issue を機械行なしで着手させる**経路そのもので、キーの作り方（起票先 repo を `issueKeys` に
通す）も、手元に無い item id の1回だけの引き方も、起票直後でボード登録が追いついていないときの
扱いも、全部そこにある。**この経路で違うのはこれだけ**:
...
- **「着手へ渡すとき」の突き合わせは要らない。** 秒前に起票した issue には、他人のアサインも
  worktree も PR も無い — あそこが機械行から読む列には見るものが無いので、機械行が無くて困らない。
  **キー未設定の行にもならない**（起票先は親 issue のある repo で、この hub は識別子をその
  `issueKeys` で逆引きして立っている。「親タスクの hub」の冒頭）。
- **渡すのは1件。** 残りは起票したまま置く。全部にタブを立てても人は捌けないし、どれを先に
  やるかは hub の決めることではない（同じく「順序を発明しない」）。

**質問を閉じたあとにキーを言われたら、収集は待たない。** 「1. タスクを選ぶ」が名指しされた1件を
「その1件だけを直接引いて進める」と書いているのと同じで、**引いた値で「着手へ渡すとき」の
突き合わせをやる** — アサインとステータスは issue から、PR は
`gh pr list -R <codeRepo> --search '{キー}' --state all`（「親の下を引く」の取りこぼしの拾い方）、
worktree は proctor か `git worktree list` から。**上の免除が効くのはこの質問の場だけ。** 他人の
アサインも worktree も PR も無いと言えるのは秒前に起票したからで、時間が経てば誰かが取っている。
