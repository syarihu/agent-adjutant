...
**番号はこの一覧の行に当てる。** 決まったキーと、収集の機械行がその行に持っている repo・item id を
そのまま持って「2. 着手を宣言する」へ進み、**「1. タスクを選ぶ」で一覧を作り直さない** — あちらは
...
**それでも取りこぼす。** 本文に `Closes #233` としか書いておらず、ブランチ名にもキーが無い PR は
どちらの引き方にも当たらない。**独自の `branchPattern` を持つソースも同じ** — `--head` は完全一致
なので、`{user}/{キー}` でない形のブランチには当たらない（キーが名前のどこかに入っていれば
`--search` のほうで拾えることもあるが、あれが見るのはタイトルと本文で、ブランチ名ではない）。
取りこぼしたサブタスクは PR 無しとして並ぶので、**「PR が無い」を
「まだ誰も手を付けていない」と読み替えない。**
...
- worktree も同じ要領で1件1行にするのだ（`proctor worktree ls --json`、無ければ
  `git worktree list`）。どのサブタスクのものかはブランチ名の中のキーで取るのだ
  （`{user}/{サブタスクのキー}`、`linear` は `gitBranchName`）。**この形は既定であって決まりでは
  ないのだ** — ソースが独自の `branchPattern` を持っていれば違う形になるので、丸ごと一致ではなく
  ブランチ名にキーが入っているかで突き合わせるのだ
