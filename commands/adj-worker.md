...
- `commands` に `verify` の各コマンドを、結果・かかった時間・出力（長ければ末尾）付きで入れる。
  ...
  直して回し直し、直せなかったものは `fail` のまま残す。**ここでコードを直したら §3 に戻る** —
  セルフレビュー・コミット・diff の gate か記録をやり直してから §4 を始め直す。そうしないと、
  レビューした差分と出す差分が食い違う。
...
- `commands` の `result` は `pass` / `fail`。`manual` は人が見るチェックリストで、1件1行。
